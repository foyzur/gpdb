-- @product_version hawq: [1.2.2.0-]
-- create view to read the segspace value

CREATE EXTERNAL WEB TABLE hawq_toolkit.__hawq_localid
(
    localid    int
)
EXECUTE E'echo $GP_SEGMENT_ID' FORMAT 'TEXT';

CREATE EXTERNAL WEB TABLE hawq_toolkit.__hawq_masterid
(
    masterid    int
)
...
SELECT C.*
FROM hawq_toolkit.__hawq_localid, mpp_23802_gp_workfile_segspace_f() AS C (
segid int,
size bigint
)
UNION ALL
SELECT C.*
FROM hawq_toolkit.__hawq_masterid, mpp_23802_gp_workfile_segspace_f() AS C (
segid int,
size bigint
);
...
SELECT * FROM hawq_toolkit.__hawq_localid, mpp_23802_gp_workfile_mgr_reset_segspace_f() 
UNION ALL
SELECT * FROM hawq_toolkit.__hawq_masterid, mpp_23802_gp_workfile_mgr_reset_segspace_f();
