...
class OOMTestCase(MPPTestCase, ScenarioTestCase):
    ...
    def setUp(self):
        ...
        if self.name == 'OOMTestCase.test_06_OOM_massivequery':
            ...
            Command('Run gpconfig to set GUC gp_vmem_protect_limit' ,
                    'source $GPHOME/greenplum_path.sh;gpconfig -c gp_vmem_limit_per_query -v "100MB" --skipvalidation').run(validateAfter=True)
        else:
            Command('Run gpconfig to set GUC gp_vmem_protect_limit' ,
                    'source $GPHOME/greenplum_path.sh;gpconfig -c gp_vmem_protect_limit -m 100 -v 100').run(validateAfter=True)
